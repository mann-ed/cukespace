...
public class CukeSpace extends Arquillian {
    ...
    protected Description describeChild(final FrameworkMethod method) {
        if (!Boolean.getBoolean("cukespace.runner.standard-describe")
            ...
            return Description.createTestDescription(
                    InstanceControlledFrameworkMethod.class.cast(method).getOriginalClass(), CukeSpace.RUN_CUCUMBER_MTD,
    ...
    protected List<FrameworkMethod> computeTestMethods() {
        ...
        // run @Test methods
        for (final FrameworkMethod each : CukeSpace.super.computeTestMethods()) {
            // methods.add(each);
        }
        // hello ed
    ...
    public static class InstanceControlledFrameworkMethod extends FrameworkMethod {
        private final CukeSpace instance;
        private final Class<?>  originalClass;
        private RunNotifier     notifier;
        private ParentRunner<?> child;

        private InstanceControlledFrameworkMethod(final CukeSpace runner, final Class<?> originalClass,
