package com.github.cukespace.config;
...
public class CucumberConfiguration {
    ...
    private boolean                            report;                                                   // jenkins
                                                                                                         // report
    private boolean                            generateDocs;                                             // cukedoctor
                                                                                                         // report
    private boolean                            generateDocsAsHtml         = true /* for compat */;       // should
                                                                                                         // cukedoctor
                                                                                                         // report be
                                                                                                         // converted
                                                                                                         // in
    ...
    private static final ThreadLocal<Class<?>> localClass                 = new ThreadLocal<>();
    ...
    // raw configuration in arquillian.xml for internal "extensions" (adoc uses it
    // to extract attributes for instance)
    ...
    public BeanManager getBeanManager() {
        return this.beanManagerInst.get();
    }
    ...
    public static CucumberConfiguration from(final Map<String, String> properties,
            final Instance<BeanManager> beanManagerInst) {
        synchronized (CONFIGURATION) { // could it really be multithreaded?
            ...
            CONFIGURATION.beanManagerInst = beanManagerInst;
        }
    ...
    public static void reset() {
        ...
        CONFIGURATION.beanManagerInst = null;
    ...
    public Properties getConfigurationAsProperties() {
        ...
        if (this.isInitialized()) {
            configurationProperties.setProperty(CucumberConfiguration.PERSISTENCE_EVENTS,
                    Boolean.toString(persistenceEventsActivated));
            configurationProperties.setProperty(CucumberConfiguration.COLORS, Boolean.toString(colorized));
            configurationProperties.setProperty(CucumberConfiguration.REPORTABLE,
                    Boolean.toString(report || generateDocs));
    ...
    public void setLocalClass(final Class<?> clazz) {
        if (null == localClass.get()) {
            localClass.set(clazz);
        }
    }
