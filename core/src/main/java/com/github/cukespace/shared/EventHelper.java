...
import com.github.cukespace.api.event.AfterAfterHooks;
import com.github.cukespace.api.event.AfterBeforeHooks;
import com.github.cukespace.api.event.BeforeAfterHooks;
import com.github.cukespace.api.event.BeforeBeforeHooks;
...
public class EventHelper {
    ...
    /*
     * public static void matched(final StepDefinitionMatch match) { final TestEvent
     * event; try { final Field field =
     * StepDefinitionMatch.class.getDeclaredField("stepDefinition"); if
     * (!field.isAccessible()) { field.setAccessible(true); }
     *
     * final StepDefinition stepDefinition =
     * StepDefinition.class.cast(field.get(match)); if
     * (ArquillianStepDefinition.class.isInstance(stepDefinition)) { final
     * ArquillianStepDefinition arquillianStepDefinition =
     * ArquillianStepDefinition.class .cast(stepDefinition); event = new
     * TestEvent(arquillianStepDefinition.getInstance(),
     * arquillianStepDefinition.getMethod()); TEST_EVENT.set(event); } else { //
     * mock to still fire events but just as marker, TODO: don't use TestEvent? ->
     * // java 8 event = new TestEvent(TEST_EVENT,
     * EventHelper.class.getMethod("currentEvent")); } } catch (final Exception e) {
     * throw new IllegalStateException(e); } TEST_EVENT.set(event); }
     */
