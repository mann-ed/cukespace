...
import com.github.cukespace.api.event.AfterStep;
import com.github.cukespace.api.event.BeforeStep;
...
public class PersistenceExtensionIntegration {
    ...
    public static boolean isOn() {
        if (IS_ON == null) {
            try {
                Class.forName("org.jboss.arquillian.persistence.core.event.BeforePersistenceTest", false,
    ...
    public static class Observer {
        ...
        public void before(@Observes final BeforeStep event) {
            ...
            final boolean           isActive  = new PersistenceExtensionEnabler(extractor)
                    .shouldPersistenceExtensionBeActivated();
            active.set(isActive);

            if (isActive) {
                final PersistenceConfiguration persistenceConfiguration = configurationInstance.get();
                persistenceExtensionFeatureResolverProvider.set(new PersistenceExtensionFeatureResolver(
                        event.getTestMethod(), extractor, persistenceConfiguration));
                persistenceExtensionScriptingFeatureResolverProvider
                        .set(new PersistenceExtensionScriptingFeatureResolver(event.getTestMethod(), extractor,
